...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  holder.tv_banzhuren = (TextView)convertView.findViewById(R.id.tv_banzhuren);
	  /*设置各个控件的展示内容*/
	  ...
	  holder.tv_collegeName.setText("所在学院：" + mData.get(position).get("collegeName").toString());
	  holder.tv_specialName.setText("所属专业：" + mData.get(position).get("specialName").toString());
	  try {holder.tv_bornDate.setText("成立日期：" + mData.get(position).get("bornDate").toString().substring(0, 10));} catch(Exception ex){}
	  holder.tv_banzhuren.setText("班主任：" + mData.get(position).get("banzhuren").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
