...
public class ClassInfoAction extends BaseAction {
    ...
    public String QueryClassInfo() {
        ...
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNo, className, collegeName, specialName, currentPage);
        /*计算总的页数和总的记录数*/
        classInfoDAO.CalculateTotalPageAndRecordNumber(classNo, className, collegeName, specialName);
        /*获取到总的页码数目*/
        ...
        ctx.put("collegeName", collegeName);
    ...
    public String QueryClassInfoOutputToExcel() { 
        ...
        if(collegeName == null) collegeName = "";
        if(specialName == null) specialName = "";
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNo,className,collegeName,specialName);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","班级名称","所在学院","所属专业","成立日期","班主任"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNo(),classInfo.getClassName(),classInfo.getCollegeName(),classInfo.getSpecialName(),new SimpleDateFormat("yyyy-MM-dd").format(classInfo.getBornDate()),classInfo.getBanzhuren()});
        }
        /*
